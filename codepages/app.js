...
function togglePricePanel() {
    ...
    var toggle = document.getElementById('price-list-toggle');
    
    if (pricePanelOpen) {
        panel.classList.add('open');
        main.classList.add('panel-open');
        toggle.classList.add('hidden');
        renderPricePanelList();
    } else {
...
function renderPricePanelList() {
    var c = document.getElementById('price-panel-list');
    if (!AppState.products.length) {
        c.innerHTML = '<div style="padding:40px 20px;text-align:center;color:var(--text-muted)">Loading products...</div>';
        return;
    }
        ...
        <div class="price-panel-item" data-type="${p.assetType||''}" data-name="${p.name.toLowerCase()}">
            <div class="price-panel-item-info">
                ...
                <div class="price-panel-item-meta">
                    ...
                    <span>·</span>
                    <span>${p.unit}</span>
                    ${p.assetType ? `<span class="badge-type ${p.assetType.toLowerCase().replace(/\\s+/g,'-')}">${p.assetType}</span>` : ''}
                </div>
            </div>
...
function filterPanelProducts() {
    var search = document.getElementById('panel-price-search').value.toLowerCase();
    var type = document.getElementById('panel-price-type').value;
